...
물리 제약이 무결성의 최종 백스톱이고, 쓰기 파이프라인 검증이 그 앞단에서 같은 규칙을 검사해
호출자 오류를 4xx 로 번역한다 (2026-07-22 헌법 개정과 정합).

| 처리 방식 | 적용 기준 | 예시 |
|-----------|-----------|------|
| **Physical** | 데이터 무결성 — 동시성 하에서 DB 만이 보증 가능 | PK, 인덱스, FK, NOT NULL, UNIQUE, DEFAULT |
| **Virtual (앱 계층 전용)** | 표현식이 논리명 세계에 살아야 하는 것 | CHECK |

이 접근법의 핵심 이점:

- **친절한 오류 표면**: 물리 위반 전에 파이프라인이 컬럼명을 담은 400 으로 응답
- **개명 유연성**: CHECK 표현식이 물리명에 결속되지 않아 이름 변경이 메타데이터 갱신으로 끝남
- **Bulk 작업 최적화**: 대량 import 시 파이프라인 검증 수위 조절 가능(물리 백스톱은 유지)
