...
> bring it. Everything else is in the published version, **0.12.3** — the one the README pins.
> To read the reference for a release rather than for `main`, open this file at its tag:
> `docs/API.md` at `v0.12.3`.
>
...
> version the published one has not reached, and that the version named above is the one the README
> pins — so a release cannot quietly leave either of them behind.
...
**The signing secret is generated by the server, not supplied by the caller** — it is not a request
field. The create response carries it once:

```json
{ "id": "…", "secret": "1f6b3fb9e8a0814c…", "isActive": true, "…": "…" }
```
