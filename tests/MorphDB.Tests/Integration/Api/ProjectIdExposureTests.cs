...
public class ProjectIdExposureTests
{
    ...
    public async Task GraphQl_schema_queries_do_not_list_project_id_as_a_column()
    {
        ...
        const string query = """
            ...
            """;
        var response = await _client.PostAsJsonAsync("/graphql", new { query, variables = new { name = tableName } });
        response.EnsureSuccessStatusCode();
    ...
    public async Task A_default_csv_export_does_not_carry_the_project_id()
    {
        ...
        var job = await bulkService.StartCsvExportAsync(_fixture.Api.ProjectId, tableName, new CsvExportOptions { IncludeHeader = true });
        using var output = new MemoryStream();
        await bulkService.StreamExportAsync(job.JobId, output);
        output.Position = 0;
        var header = (await new StreamReader(output).ReadLineAsync())!;
    ...
    public async Task An_export_may_not_select_the_project_id_by_name()
    {
        ...
        var response = await _client.PostAsJsonAsync($"/api/bulk/{tableName}/export/csv",
            new CsvExportApiRequest { Columns = ["grade", "project_id"] });

        response.StatusCode.Should().Be(HttpStatusCode.BadRequest, "no consumer may name a column no surface exposes");
        (await response.Content.ReadAsStringAsync()).Should().Contain("COLUMN_NOT_FOUND");
    }
    ...
    public async Task An_export_column_that_matches_no_column_is_refused_before_any_sql_runs()
    {
        ...
        var response = await _client.PostAsJsonAsync($"/api/bulk/{tableName}/export/csv",
            new CsvExportApiRequest { Columns = ["(SELECT string_agg(tablename, ',') FROM pg_tables)"] });
